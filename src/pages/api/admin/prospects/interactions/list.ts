...
// POST pour un read : viole REST mais suit la convention maison du repo
// (toutes les routes /api/admin sont POST+body JSON — voir PRD §4.2).
export const POST: APIRoute = async ({ request, cookies }) => {
  try {
    ...
    if (fetchError) {
      console.error('[list interactions]', fetchError);
      return new Response(
    ...
    return new Response(
      JSON.stringify({ success: true, interactions: interactions ?? [] }),
